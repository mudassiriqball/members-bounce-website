...
const PlayNowSearchFilter = (props) => {
        ...
        <Row>
          <div className="mb-3">
            ...
            <Select
              ...
              placeholder="Choose..."
              value={filterBy === 'date' ? filterType === 'a_z' ? { value: 'a_z', label: 'Newest First' } : { value: 'z_a', label: 'Older First' } : ""}
              onChange={(data) => {
                setFilterBy('date');
                setFilterType(data.value);
              }}
              options={[
                { value: "a_z", label: "Newest First" },
                { value: "z_a", label: "Older First" },
              ]}
            />
          </div>
          <div className="mb-3">
            ...
            <Select
              classNamePrefix="select2-selection"
              placeholder="Choose..."
              value={filterBy === 'club' ? filterType === 'a_z' ? { value: 'a_z', label: 'A-Z' } : { value: 'z_a', label: 'Z-A' } : ""}
              onChange={(data) => {
                setFilterBy('club');
                setFilterType(data.value);
              }}
...
export default PlayNowSearchFilter;
